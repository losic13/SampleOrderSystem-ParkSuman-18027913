...
| **SampleOrderSystem** (이 저장소) | 미션2 — 4개 PoC 설계를 계승한 최종 통합 시스템 | **진행 중 — 문서/스캐폴딩 + 하네스 구축 + 태스크3(도메인 모델) 계획 문서화까지 커밋됨, 도메인 코드 구현 시작 전** |
...
문서/스캐폴딩(1번), 하네스 구축(2번), 태스크3(도메인 모델) 계획 문서화(`docs/plans/03-domain-model-plan.md`)
까지 완료된 상태이며, 다음은 태스크3의 Red-Green-Review 사이클(3a RED부터) 진행이다.
...
3. ⬜ 도메인 모델 (`model/`) — 계획 문서화(`docs/plans/03-domain-model-plan.md`) ✅ 완료
...
- 이 저장소는 아직 `model/`/`repository/`/`controller/`/`monitor/`/`view/`/`main.py`/`tests/`가 전혀 없다
  (문서/스캐폴딩 + 하네스 + 태스크3 계획 문서화까지만 커밋됨). `CONVENTIONS.md`의 코드 스니펫을 그대로
  재사용해 작성 시작하면 된다.
