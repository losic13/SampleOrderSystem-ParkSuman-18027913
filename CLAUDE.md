...
| **SampleOrderSystem** (이 저장소) | 미션2 — 4개 PoC 설계를 계승한 최종 통합 시스템 | **완료 — 태스크1~10 전부 커밋됨 (도메인 모델/저장소/컨트롤러/생산라인/모니터링/콘솔 뷰/종단테스트/문서화)** |
...
`architecture-guardian` 프로젝트 스킬)를 먼저 구축**했다 (아래 "Harness" 섹션 참고). 이 방식으로
태스크1~10을 전부 완료했다 — 상세 이력은 아래 "커밋 계획"과 `git log --oneline` 참고.
...
model/
  ...
  production_job.py                          # order_id, sample_id, shortage, actual_quantity,
...
repository/
  ...
  production_job_repository.py                # order_id를 dict 키로 사용 (FIFO = insertion order)
controller/
  sample_controller.py                        # ConsoleMVC와 동일 (register/list_all/search_by_name)
  order_controller.py                         # ConsoleMVC 계승 + 재고부족 분기에서 ProductionController.enqueue
                                               호출 (생성자 주입, 단방향 의존 — ProductionController는
                                               OrderController를 모른다). place_order/list_reserved/
                                               list_confirmed/approve/reject/complete_production/release.
                                               approve/reject/release는 존재하지 않는 order_id에 ValueError
                                               (main.py 스모크 테스트로 발견해 태스크9에서 추가)
  production_controller.py                    # enqueue/list_queue/current_job/advance_time(minutes) —
                                               advance_time은 현재 작업만 tick하고 완료된 job 목록만
                                               반환한다 (OrderController.complete_production 호출은
                                               view/main.py 책임 — 단방향 의존 유지)
monitor/
...
2. ✅ **개발 하네스 구축** — `architecture-guardian`/`verify` 프로젝트 스킬, Red-Green-Review 커밋 규율 확정
3. ✅ 도메인 모델 (`model/`) — 계획(`docs/plans/03-domain-model-plan.md`) → RED → GREEN → REVIEW
   (커버리지 갭 1건 보완, `architecture-guardian`/`code-review`/`simplify` 위반 없음)
4. ✅ JSON 저장소 계층 (`repository/` — sample/order/production job) — 계획(`04-repository-plan.md`)
   → RED → GREEN (REVIEW 지적사항 없어 생략)
5. ✅ 시료/주문 컨트롤러 (`controller/sample_,order_` — 재고부족시 재고 즉시 0 차감 포함) —
   계획(`05-controller-plan.md`) → RED → GREEN → REVIEW (`list_reserved` 커버리지 갭 보완).
   이후 태스크9 스모크 테스트에서 `list_confirmed`/존재하지 않는 order_id 검증 누락을 추가로 발견해
   각각 별도 RED→GREEN으로 보완
6. ✅ 생산 컨트롤러 (`controller/production_` — FIFO tick 시뮬레이션) — 계획
   (`06-production-controller-plan.md`) → RED → GREEN → REVIEW (빈 큐 분기 커버리지 갭 보완)
7. ✅ 모니터링 서비스 (`monitor/`) — 계획(`07-monitor-plan.md`) → RED → GREEN (REVIEW 지적사항 없어 생략)
8. ✅ 콘솔 뷰 + 진입점 (`view/`, `main.py`) + 수동 시나리오 검증 (1커밋 — 전체 생명주기 파이프 스모크로 확인)
9. ✅ 종단 시나리오 테스트 — 계획(`09-end-to-end-plan.md`) → RED(assertion 오류로 실패) → GREEN
   (assertion 수정 + `main.py` 스모크에서 발견한 버그 수정) → REVIEW(`/verify` 1차 전체 호출,
   `architecture-guardian`/`code-review`/`simplify` 위반 없음)
10. ✅ CLAUDE.md 최종화(전체 구현 반영) + README.md (1커밋)

각 커밋 전 `pytest -v` 통과를 확인했다 (RED 커밋은 예외 — 의도적으로 실패하는 테스트를 커밋).
push는 매번 진행 후 사용자에게 보고했다 (기존 4개 저장소와 동일 패턴).
...
python -m pytest -v         # 단위/종단 테스트 (64개, 전부 통과)
python -m pytest --cov      # 커버리지 리포트 (model/repository/controller/monitor 100%,
                             # view는 콘솔 I/O라 수동 시나리오로만 검증 — pytest 커버리지 대상 아님)
```

Windows 콘솔 codepage(CP949) 한글 깨짐 이슈는 다른 저장소와 동일하게 `main.py`의
`sys.stdout/stdin.reconfigure(encoding="utf-8")`로 처리한다. Bash 도구로 직접 실행해 검증할 때는
...
- `model/`/`repository/`/`controller/`/`monitor/`/`view/`/`main.py`/`tests/` 전부 구현 완료. 계층별
  상세 설계 근거는 `docs/plans/0N-xxx-plan.md`, 진행 현황 스냅샷은 `docs/plans/TASKS.md` 참고.
